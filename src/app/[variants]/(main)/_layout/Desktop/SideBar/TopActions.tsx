...
import { Brain, Compass, FolderClosed, MessageSquare } from 'lucide-react';
...
const TopActions = memo<TopActionProps>(({ tab, isPinned }) => {
  ...
  const isMindfulnessActive = tab === SidebarTabKey.Mindfulness;
  const isEmotionActive = tab === 'emotion-dashboard';
  ...
  return (
    <Flexbox gap={8}>
      ...
      {/* 情绪管家菜单项 */}
      <Link aria-label={'情绪管家'} href={'/main/emotion-dashboard'}>
        <ActionIcon
          active={isEmotionActive}
          icon={Brain}
          size={ICON_SIZE}
          title={'情绪管家'}
          tooltipProps={{ placement: 'right' }}
      ...
      <Link aria-label={t('tab.mindfulness')} href={'/main/mindfulness'}>
        <ActionIcon
          active={isMindfulnessActive}
          icon={Brain}
          size={ICON_SIZE}
          title={t('tab.mindfulness')}
          tooltipProps={{ placement: 'right' }}
        />
      </Link>
    </Flexbox>
